import { initialize, update } from "./instructions";
...
export const getMatch = async (
  ...
  return await web3.PublicKey.findProgramAddress(
    [Buffer.from("pancake"), me.toBuffer(), Buffer.from(epoch.toString())],
    PROGRAM_ID
  );
};
...
async function main() {
  let wallet = Keypair.fromSecretKey(
    new Uint8Array(
      JSON.parse(
        fs.readFileSync("/home/gitpod/.config/solana/id.json").toString()
      )
    )
  );
  ...
  setInterval(async function () {
    ...
    try {
        ...
        if (theEpoch != currentEpoch) {
         ...
         // await connection.requestAirdrop(predictions, 1 * 10 ** 9)  
          let ini = await initialize(
            { epoch: currentEpoch },
            {
          ...
          tx.add(ini)
          signers.push(predictions)
        }
  
        if (stuff.length > 0) {
        let upd =  await update(
            {
              epoch: currentEpoch,
              bull: stuff[stuff.length - 1],
              bear: stuffbear[stuffbear.length - 1],
            },
            { predictions: predictions, auth: wallet.publicKey }
          );
          tx.add(upd)
        }
        try {
            signers.push(wallet)
            prov2.sendAndConfirm(tx, signers)
        }
        catch (err){
            console.log(err)
        }
